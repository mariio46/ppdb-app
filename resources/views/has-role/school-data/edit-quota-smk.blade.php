            ...
            <div class="card-body">
                <form id="form-add-quota" action="#" method="POST">
                    ...
                    <div class="d-flex align-items-center justify-content-start gap-2 mt-2">
                        ...
                        <x-link color="secondary" :href="route('school-data.quota')">Batalkan</x-link>
                    </div>
                </form>
            </div>
        </div>
...
@push('scripts')
    ...
    <script>
        $(function() {
            ...
            $.ajax({
                url: `/panel/data-sekolah/json/major-quota/${identifier}`,
                method: 'get',
                dataType: 'json',
                success: function(quota) {
                    $('#delete-modal-title').text(`Hapus Jurusan ${quota.label}`)
                    $('#jurusan').val(quota.label)
                    if (quota.value) {
                        let totalDividing = quota.value / default_value;
